...
func insertTrapPoints() {
	...
	// 	if strings.Contains(fn.Sym().Name, "_DumpIR") {
	// 		ir.Dump("debug:", fn)
	// 		return
	// 	}
	// }
	...
	trap := typecheck.LookupRuntime("__x_trap")
	for _, fn := range typecheck.Target.Funcs {
		// fn.Body =
		t := fn.Type()
		...
		// condExpr :=
		// stmt := ir.NewIfStmt(base.AutogeneratedPos, callExpr, nil, fn.Body)

		fn.Body = []ir.Node{assignStmt, typecheck.Stmt(ifStmt)}
		// if strings.Contains(os.Getenv("COMPILER_DEBUG_IR_REWRITE_FUNC"), fn.Sym().Name) {
		// 	ir.Dump("debug before:", fn)
		// }
		typecheck.Stmts(fn.Body)
		// if strings.Contains(os.Getenv("COMPILER_DEBUG_IR_REWRITE_FUNC"), fn.Sym().Name) {
		// ir.Dump("debug after:", fn)
		// }
	}
}
